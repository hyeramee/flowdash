/* =================================================== */
/* 헤더 초기화 (다크모드 토글 + 환영 아이콘 로테이션)     */
/* =================================================== */

// 공통 스토리지 함수와 연동 상수를 최상단에 불러옵니다.
import { loadTheme, saveTheme } from '../storage.js';
import { getState, setState } from '../state.js'; // ⭕ 팀 피드백 전역 상태 함수 연동을 위해 추가
...
export function initHeader() {
  ...
  if (toggleBtn) {
    // 1. [최초 기억] HTML에 이미 들어있는 달 SVG 코드를 복사해서 기억해 둡니다.
    ...
    // 3. [초기화] 공통 loadTheme 함수와 THEME 상수를 연동하여 적용합니다.
    ...
    document.body.addEventListener('click', (event) => {
      ...
      if (isDarkMode) {
        saveTheme(THEME.DARK); // THEME.DARK 상수를 활용해 저장합니다.
        updateIcon(true);
      } else {
        saveTheme(THEME.LIGHT); //THEME.LIGHT 상수를 활용해 저장합니다.
        updateIcon(false);
      }
    });
  }
  ...
  if (iconContainer) {
    ...
    function updateWelcomeIcon(stateNum) {
      if (stateNum === 1) {
        iconContainer.innerHTML = icon1_rocket;
      } else if (stateNum === 2) {
        iconContainer.innerHTML = icon2_star;
      } else {
        iconContainer.innerHTML = icon0_default;
      }
    }

    // 로테이션 인덱스 처리 (별도 요구사항이 없는 한 기존 로직 고수 -> 피드백 전역 상태 규격으로 리팩토링)
    const { welcomeIndex } = getState(); // ⭕ 전역 상태에서 인덱스 수신
    const welcomeCurrentIndex = welcomeIndex ?? 0;
    const welcomeNextIndex = (welcomeCurrentIndex + 1) % 3;
    setState({ welcomeIndex: welcomeNextIndex }); // ⭕ 전역 상태를 통해 자동 저장 관리
  ...
  /* -------------------------------------------------- */
  /* [파트 3] 사용자 이름 로컬스토리지 저장 로직            */
  /* -------------------------------------------------- */
  const nameDOM = document.querySelector('.title-area .click-purple');

  if (nameDOM) {
    // 1. [초기화] 공통 loadNickname 함수 연동 -> 피드백 전역 상태 규격으로 리팩토링
    const { nickname } = getState(); // ⭕ 전역 상태에서 닉네임 수신
    if (nickname) {
      nameDOM.textContent = nickname;
    }

    // 2. [이벤트] 입력창 바깥을 클릭하여 수정을 마쳤을 때(blur) 공통 saveNickname 함수 연동 -> 피드백 전역 상태 규격으로 리팩토링
    nameDOM.addEventListener('blur', () => {
      const currentName = nameDOM.textContent.trim();

      if (currentName) {
        setState({ nickname: currentName }); // ⭕ 전역 상태 규칙 적용
      } else {
        // 내용을 전부 지우고 비워뒀다면 기본값으로 복구
        nameDOM.textContent = 'FlowDash';
        setState({ nickname: 'FlowDash' }); // ⭕ 전역 상태 규칙 적용
      }
    });
  }
}
