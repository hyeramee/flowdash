...
export function initHeader() {
  ...
  if (toggleBtn) {
    // 1. HTML에 이미 들어있는 달 SVG 코드를 복사해서 기억해 둡니다.
    ...
    // 아이콘 동기화 부분도 위에서 구한 `isDarkTheme`를 재활용합니다.
    if (isDarkTheme) {
      updateIcon(true);
    }
  ...
  /* -------------------------------------------------- */
  /* ⚡ [타이머] 로딩 시차 조율 (완벽 동기화 수정)          */
  /* -------------------------------------------------- */

  // body와 board를 분리하지 않고, 동기적으로 body에 클래스를 즉시 주입합니다.
  // 이로 인해 CSS에서 body.theme-loaded 하나만으로 위아래를 동시에 컨트롤할 수 있게 됩니다.
  document.body.classList.add('theme-loaded');

  // 기존에 있던 #board 관련 setTimeout은 완전히 지우거나,
  // 만약 다른 곳에서 board-ready 클래스를 꼭 써야만 한다면 아래처럼 동기적으로 한 번에 붙여줍니다.
